...
class DocTool {
    ...
    createImgInLineWithBase64(base64, obj = {type: "png"}) {
        if (this.creating) {
            this.taskQueue[this.taskQueue.length] = {
                method: this.createImgInLineWithBase64,
    ...
    createImgFloatWithBase64(base64, obj = {type: "png", float: "right"}) {
        if (this.creating) {
            this.taskQueue[this.taskQueue.length] = {
                method: this.createImgFloatWithBase64,
                argArray: Array.from(arguments)
            };
        } else {
            this.creating = true;
    ...
    createImgInLineWithPath(path, obj = {}) {
        if (this.creating) {
            this.taskQueue[this.taskQueue.length] = {
                method: this.createImgInLineWithPath,
                argArray: Array.from(arguments)
            };
        } else {
            this.creating = true;
    ...
    createImgFloatWithPath(path, obj = {float: "right"}) {
        if (this.creating) {
            this.taskQueue[this.taskQueue.length] = {
                method: this.createImgFloatWithPath,
                argArray: Array.from(arguments)
            };
        } else {
            this.creating = true;
     ...
     * @param text 多行文本
     * @param delimiter 分割符
     * @returns {number}    没有分割符返回-1
     */

    judge(text, delimiter) {
        if (delimiter !== undefined) {
            return text.indexOf(delimiter);
        } else {
            return text.indexOf("\n");
        }
    }
